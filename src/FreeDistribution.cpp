...
void Distribution::set_basis(size_t step, GridSpacing grid_style, Cutoffs param_cutoffs, FeatureRegimes regimes, DynamicGridPreset dyn_grid_preset,ManualGridBoundaries elec_grid_regions){
...
int Distribution::seek_basis(ofstream& _log, GridSpacing gt, size_t step, Cutoffs param_cutoffs){
    ...
    //3. If error in each region is lower than tolerable, we are done, otherwise:
...
int Distribution::adapt_knots(bool init, GridSpacing gt, Distribution original_distribution, std::vector<double> original_knots, std::vector<double> reference_energies, std::vector<double> prev_Edens, size_t iteration){
    return 1;
    // double MAX_ERR = 0.00001;  
    // size_t MAX_NUM_KNOTS = 275;
    // // todo max knots per region? 
    // if (init){
    //     // // transform 
    ...
    //     reference_energies = {};
    //     prev_Edens = {};
    //     size_t starting_point_count = 2;
    //     for(size_t i = 0; i < basis.regions.size();i++){
    //         basis.regions[i].set_num_points(starting_point_count);
    //     }
    //     size_t reference_pts_per_knot = 4000/Distribution::get_knot_energies().size();
    //     // Using the current distribution's knots, generate a number of energy points that will serve as our test points for the mean square difference.
    //     auto current_knots = Distribution::get_trimmed_knots(Distribution::get_knot_energies());
    //         for (size_t i=0; i<current_knots.size()-1; i++){
    //         double e = current_knots[i];
    //         double de = (current_knots[i+1] - current_knots[i])/(reference_pts_per_knot-1);
    //         for (size_t j=0; j<reference_pts_per_knot; j++) {
    //             reference_energies.push_back(e);
    //         e += de;
    //         prev_Edens.push_back(INFINITY);
    //         }        
    //     }
    // }

    // FeatureRegimes dummy_regimes; 
    // auto new_knots = basis.set_knot(gt,dummy_regimes,true,true);
    // transform_basis(new_knots);
    
    // // add knots to regions that have too high NMS differences with the previous iteraton.
    // bool exceeded_error = false;
    // std::vector<double> Edens;
    // std::vector<double> reference_Edens;
    // for (double e : reference_energies){ 
    //     Edens.push_back((*this)(e)*e);
    //     reference_Edens.push_back(original_distribution(e)*e);
    // }
    ...
    //         // segment_square_diff += pow(Edens[j] - reference_Edens[j],2);
    //         // segment_square_denom += pow(reference_Edens[j],2);
    //     }

    //     double norm_MSD = segment_square_diff/segment_square_denom;

    //     if(norm_MSD > MAX_ERR || isnan(norm_MSD)){
    //         exceeded_error = true;
    //         basis.regions[i].set_num_points(basis.regions[i].get_num_points() + 1);
    //     }
    // }
    // iteration++;
    // if (get_knot_energies().size() > MAX_NUM_KNOTS){
    //     set_distribution(original_knots,original_distribution.f);
    //     return 1;
    // }
    // // Recursively call, until convergence (i.e. when error isnt't exceeded).
    // if (exceeded_error){
    //     // reset distribution state
    //     set_distribution(original_knots,original_distribution.f);
    //     // try again
    //     return adapt_knots(false,gt,original_distribution,original_knots, reference_energies,Edens,iteration);
    // }
...
//             basis.regions[i].set_num_points(starting_point_count);
//         }
//         size_t reference_pts_per_knot = 3;
//         // Using the current distribution's knots, generate a number of energy points that will serve as our test points for the mean square difference.
//         auto current_knots = Distribution::get_trimmed_knots(Distribution::get_knot_energies());
//             for (size_t i=0; i<current_knots.size()-1; i++){
//             double e = current_knots[i];
...
//     double biggest_norm_MSD = 0;
//     size_t segment_to_add_knot = -1;
//     for(size_t i = 0; i < basis.regions.size();i++){
//         size_t min_knot = basis.i_from_e(basis.regions[i].get_E_min()); 
//         size_t max_knot = basis.i_from_e(basis.regions[i].get_E_max()); 
//         double segment_square_diff = 0;
...
//         for(size_t j = min_knot; j <= max_knot;j++){
//             double e = get_knot_energies()[i];
//             segment_square_diff += pow(Edens[j] - reference_Edens[j],2);  /// probs not possible to get error low w/ this due to overlapping
//             segment_square_denom += pow(reference_Edens[j],2);          
//             // segment_square_diff += pow(Edens[j] - prev_Edens[j],2);
//             // segment_square_denom += pow(prev_Edens[j],2);
//         }
...
//         if(norm_MSD > MAX_ERR || isnan(norm_MSD)){
//             exceeded_error = true;
//             if (norm_MSD >= biggest_norm_MSD || isnan(norm_MSD)){
//                 if(isnan(norm_MSD)){
//                     biggest_norm_MSD = INFINITY;
//                 }
//                 else{
//                     biggest_norm_MSD = norm_MSD;
//                 }
//                 segment_to_add_knot = i;
//             }
            
//         }
//     }
//     iteration++;
//     if (iteration > MAX_NUM_KNOTS || get_knot_energies().size() > MAX_NUM_KNOTS){
//         set_distribution(original_knots,original_distribution.f);
//         return 1;
//     }
//     // Recursively call, until convergence (i.e. when error isnt't exceeded).
//     if (exceeded_error){
//         basis.regions[segment_to_add_knot].set_num_points(basis.regions[segment_to_add_knot].get_num_points() + 2);
//         // reset distribution state
//         set_distribution(original_knots,original_distribution.f);
//         // try again
//         return adapt_knots(false,gt,original_distribution,original_knots, reference_energies,prev_Edens,iteration);
//     }
//     else return 0; // success!
// }
...
//     if (exceeded_error){
//         basis.regions[segment_to_add_knot].set_num_points(basis.regions[segment_to_add_knot].get_num_points() + 2);
//         // make change to part of prev_e_dens corresponding to knots that were updated (bit rough)
//         set_distribution(original_knots,original_distribution.f);
//         auto new_knots2 = basis.set_knot(gt,dummy_regimes,true,true);
//         transform_basis(new_knots2);
//         size_t min_knot2 = basis.i_from_e(basis.regions[segment_to_add_knot].get_E_min()); 
//         size_t max_knot2 = basis.i_from_e(basis.regions[segment_to_add_knot].get_E_max());    
//         for(size_t j = min_knot2; j <= max_knot2;j++){
//             double e = reference_energies[j];  
//             prev_Edens[j] = (*this)(e)*e;
//         }
