...
def main():
    ...
    make_some_plots(data_folders,molecular_path,label,dname_Figures,distinguish_by_color_not_line=(not DASHED_AUGER),show_legend=show_legend)

def make_some_plots(mol_names,sim_output_parent_dir, label,figure_output_dir,distinguish_by_color_not_line=True,show_legend=True,ignore_CNO=False, CNO_only=False):
    '''
    Arguments:
    ...
    if not distinguish_by_color_not_line:
        #linestyles = ["solid","dashed"]*int(len(continuums_set)/2)
        #param =(0,(2,1.5))
        param ="dashed"
        linestyles = ["solid",param]*int((len(continuums_set)+1)/2)
