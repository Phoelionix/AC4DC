...
#     # Axes
#     for t in P.SINGLE_FRAME_DICT["times"]: #TODO probably should just be one time...
#         if P.SINGLE_FRAME_DICT['y_range'] is None:
#             print("setting up y axis range automatically")
#             #ymax = 0
#             # for target in target_handles:
#             #     ymax = max(np.max(asdasd),0)
#             # for data in ipl.fig.data:
#             #     ymax = np.max(data)
#             ymax = 0
#             for target in ipl.target_data:
#                 j = np.searchsorted(target.timeData,t)
...
#                     data*= target.energyKnot
#                 ymax = np.max(data)
#             print(ymax)
                
#             P.SINGLE_FRAME_DICT['y_range'] = [0,ymax]

        
#         if P.INSET and ('tick0' not in P.INSET_DICT['axes_kwargs']['yaxis2'] or 'dtick' not in P.INSET_DICT['axes_kwargs']['yaxis2'] or P.INSET_DICT['axes_kwargs']['yaxis2']['tick0'] is None or P.INSET_DICT['axes_kwargs']['yaxis2']['dtick'] is None):
#             P.INSET_DICT['axes_kwargs']['yaxis2']['tick0'] = P.SINGLE_FRAME_DICT['y_range'][0]
#             P.INSET_DICT['axes_kwargs']['yaxis2']['dtick'] = (P.SINGLE_FRAME_DICT['y_range'][1]- _P.SINGLE_FRAME_DICT['y_range'][0])/3*2*(1+(1000-1)*_P.SCALE_DENSITY_BY_THOUSAND)
