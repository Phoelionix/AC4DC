...
def make_some_plots(mol_name,sim_output_parent_dir, label,figure_output_dir, tot_charge=False,bound_ionisation=False,free=False,free_slices=False,bound_ionisation_bar=False,orbital_densities_bar=False,photo_rates = False,split_free=False,combined_split_free=False):
    ...
    if tot_charge: 
        ...
        pl.plot_tot_charge(ylim=[0,3.5],plot_legend=False,every=1,charge_difference=True)  #TODO automatically set to charge_difference to True if starting with ions...
    ...
    if orbital_densities_bar:
        pl.plot_orbitals_bar(atoms=None,atoms_excluded=None,show_pulse_profile=False,normalise = True,
                             show_cbar_label=False, show_title=False,add_element_to_ylabel=True,
    ...
    if TIGHT_LAYOUT:
        ...
        pl.fig.subplots_adjust(left=0.105, bottom=0.22, right=0.995, top=0.99, wspace=0.2, hspace=0.4) # Custom 
        if VERTICAL_MODE:
        ...
        if HORIZONTAL_MODE:
            ...
            pl.fig.subplots_adjust(left=0.21/3, bottom=0.42, right=1-0.005/3, top=0.996, wspace=0.38, hspace=0.4) # 3 orb density plots row
