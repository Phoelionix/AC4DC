...
####
ELECTRON_DENSITY = False # Whether to use electron density for free distribution plots. Electron energy density if False
###
...
def make_some_plots(mol_name,sim_output_parent_dir, label,figure_output_dir, tot_charge=False,bound_ionisation=False,free=False,free_slices=False,bound_ionisation_bar=False,orbital_densities_bar=False,photo_rates = False,split_free=False,combined_split_free=False):
    ...
    if tot_charge: 
        ...
        S = ["lys_galli_LF_17","lys_galli_HF_15","lys_galli_LF_no_Gd_6","lys_galli_HF_no_Gd_2",]

        colours = ["tab:blue","tab:orange","tab:green","tab:purple"]
        ymax = 0.8; pl.plot_tot_charge(ylim=[0,ymax],plot_legend=False,every=1,charge_difference=True,legend_loc="best",atoms=["C","N","O","Gd_fast"],colours=colours); i =0
        #ymax = 6; pl.plot_tot_charge(ylim=[0,ymax],plot_legend=False,every=1,charge_difference=True,legend_loc="best",atoms=["C","N","O","Gd_fast"],colours=colours) ; i =1 
        #ymax = 0.8; pl.plot_tot_charge(ylim=[0,ymax],plot_legend=False,every=1,charge_difference=True,legend_loc="best",atoms=["C","N","O"],colours=colours)  ;i = 2
        #ymax = 6;pl.plot_tot_charge(ylim=[0,ymax],plot_legend=False,every=1,charge_difference=False,legend_loc="best",atoms=["C","N","O"],colours=colours)  ; i =3

        pl.num_plotted = 0
        pl2 = Plotter(S[i],sim_output_parent_dir,use_electron_density = ELECTRON_DENSITY,end_t = END_T,
                 initialise=True)
        pl2.axs = pl.axs                 
        pl2.plot_tot_charge(ylim=[0,ymax],custom_ax=pl.axs[0][0],plot_legend=False,every=1,charge_difference=True,legend_loc="best",atoms=["S"],colours=["tab:red"]); i =0
    ...
    if TIGHT_LAYOUT:
        ...
        pl.fig.subplots_adjust(left=0.15, bottom=0.28, right=0.924, top=0.97, wspace=0.16, hspace=0.55)  # split continuums
        if VERTICAL_MODE:
