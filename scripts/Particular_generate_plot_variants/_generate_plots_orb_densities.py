...
#COLUMNWIDTH = 3.34646/2 # column of document
COLUMNWIDTH = 3.34646/2


COLWIDTH = 11/16*COLUMNWIDTH # column of figure
#ROWHEIGHT = COLWIDTH*16/16 * 3


#ROWHEIGHT = COLUMNWIDTH*12/16
#ROWHEIGHT = COLUMNWIDTH*14/16
ROWHEIGHT = COLWIDTH*14.2/16
...
def make_some_plots(mol_name,sim_output_parent_dir, label,figure_output_dir, tot_charge=False,bound_ionisation=False,free=False,free_slices=False,bound_ionisation_bar=False,orbital_densities_bar=False,photo_rates = False,split_free=False,combined_split_free=False):
    ...
    load_specific_atoms = ["C","N","O","S","Gd_fast","Cl","Na"] # ["C","Fe_singleShell"]#["C","N","O","S","Gd_fast","Cl","Na"]#["Gd_fast"] #["C"]#["Fe_singleShell","C"] #None #["C","N","O"]
    ...
    if orbital_densities_bar:
        atoms = None
        #atoms = ["C","N","O","C","C","C","C"]
        pl.plot_orbitals_bar(atoms=atoms,atoms_excluded=None,show_pulse_profile=False,normalise = True,
                             show_cbar_label=True, show_title=False,add_element_to_ylabel=True,
                             show_cbar=False,show_cbar_last=True,
                             show_ylabel = False,custom_x_ticks=[-15,0,15],vmin=0)
    ...
    if TIGHT_LAYOUT:
        ...
        if HORIZONTAL_MODE:
            ...
            #rowheight 11/16 colwidth, colwidth 3.5/2
            plt.gcf().set_figwidth(7.24409436834)
            pl.fig.subplots_adjust(left=0.21*3**1.2/3/num_subplots**1.2, bottom=0.41, right=1-0.009/num_subplots, top=0.996, wspace=0.3, hspace=0.4) # Custom for row of orbital density plots 
