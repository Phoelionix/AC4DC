...
def make_some_plots(mol_name,sim_output_parent_dir, label,figure_output_dir, tot_charge=False,bound_ionisation=False,free=False,free_slices=False,bound_ionisation_bar=False,orbital_densities_bar=False,photo_rates = False,split_free=False,combined_split_free=False):
    ...
    if free_slices:
        ...
        if AC4DC:
            ...
            for (t, e, col, T, n,custom_col ) in zip(slices, thermal_cutoff_energies,colrs,custom_T, custom_n, custom_colrs):
                data = np.genfromtxt(f"scripts/royle_data/royle_B_free_{int(t+100)}.csv",delimiter=',',skip_header=1)
                # new_x = data[:,0].tolist()
                # new_y = data[:,1].tolist()
                # new_x.insert(0,0)
                # new_y.insert(0,0)
                ...
                test = maxwell(data[:,0], T/1000, 11111)
                tot = np.dot(test,de)
                test/=tot
                test/=4*3.14
                test/=1000 
