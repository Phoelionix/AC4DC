...
ROWHEIGHT = COLUMNWIDTH*14.15/16
...
def make_some_plots(mol_name,sim_output_parent_dir, label,figure_output_dir, tot_charge=False,bound_ionisation=False,free=False,free_slices=False,bound_ionisation_bar=False,orbital_densities_bar=False,photo_rates = False,split_free=False,combined_split_free=False):
    ...
    split_continuums_to_load = ["C","N","O","S","Gd_fast","Cl","Na"] #"all" #["Gd_fast"] #["C"]#["Fe_singleShell","C"]  # None is not valid. Use: [] 
    ...
    if split_free:
        ...
        if photo_then_auger and not TWO_VERTICAL_MODE:
        ...
        for _c in order:
            ...
            pl.plot_free(log=True,ylog=False,cmin=10**(-8),cmax=10**(-3.609),ylim=[0,8000],
                         ...
                         show_cbar=False,show_time_axis_label=False,show_energy_axis_label=(_c%2==0 or not TWO_VERTICAL_MODE))
    ...
    if TIGHT_LAYOUT:
        ...
        if TWO_VERTICAL_MODE:
            pl.fig.subplots_adjust(left=0.1, bottom=0.03, right=0.95, top=0.97, wspace=0.16, hspace=0.4) # Custom for column of orbital density plots 
