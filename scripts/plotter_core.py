...
class Plotter:
    ...
    def get_mol_file(self, mol):
        ...
        if not os.path.isdir(output_folder):
            raise Exception("\033[91m Cannot find simulation output folder '" + mol + "'\033[0m" )
        ...
        if path.isfile(molfile):
            if len(sys.argv) >= 4:
                y_n_check = sys.argv[3]
                unknown_response_qualifier = "argument \033[92m'"   +sys.argv[3] + "'\033[0m not understood, using default .mol file."
            else:
            ...
            if y_n_check.casefold() not in map(str.casefold,["n","no"]):  #  User didn't say to use input\
                ...
                if len(sys.argv) >= 4:
                    print('\033[95m' + "Using \033[94m'" + os.path.basename(molfile) +"'\033[95m generated by output." + '\033[0m')
            else:
