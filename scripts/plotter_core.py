...
class Plotter:
    ...
    def get_mol_file(self, mol):
        #### Inputs ####
        #Check if .mol file in outputs
        use_input_mol_file = False
        molfile = self.p + 'output/__Molecular/' + mol + '/' + mol + '.mol'
        print(molfile)
        if path.isfile(molfile):
            print('\033[95m' + "Use mol file generated by output? ('y' - yes, 'n' - use a mol file from input/ directory.)" + '\033[0m')
            ...
            if y_n_check.casefold() not in map(str.casefold,["n","no"]):  #  User didn't say to use input\
                if y_n_check.casefold() not in map(str.casefold,["y","yes"]):
                    print("Unknown response, using 'y'.")
            else:
                print("Using mol file from input/.")
                use_input_mol_file = True
        else: 
            print("\033[91mMissing input: copy of mol file used to generate output not found, searching input/ directory.\033[0m'" )
            use_input_mol_file = True
