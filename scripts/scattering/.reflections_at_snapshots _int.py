...
def reflections_at_time_slice(slice_time): 
    ...
    if laser_firing_qwargs["SPI"]:
        SPI_result1 = experiment1.fire_laser(start_time,end_time,target_handle,sim_data_dir,crystal,results_parent_dir=results1_parent_folder, **laser_firing_qwargs)
        SPI_result2 = experiment2.fire_laser(start_time,end_time,target_handle,sim_data_dir,crystal_undmged,results_parent_dir=results2_parent_folder,  **laser_firing_qwargs)
...
def plot_biggest_spots_over_time(Intensities, ideal_intensities, miller_indices,times,sqrt=False,normalize=False,num_points=4,save_path = None,title="",true_normed=False):
    plt.close()
    Intensities = copy.deepcopy(Intensities)
...
if __name__ == "__main__":
    ...
    plot_biggest_spots_over_time(Intensities,ideal_intensities,miller_indices,times,sqrt=False,num_points=num_points,save_path=dir_figures+target_handle+"_rfl.png",title=title)
