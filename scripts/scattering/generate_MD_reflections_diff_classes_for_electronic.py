...
DEBUG_WATER = False
QUICK_TEST = False
SKIP_UNDAMAGED_CONTROL = False; FIRST_UNDAMAGED=False # not the control
...
ground_truth_pdb=None if len(sys.argv)<4 else sys.argv[3] # scripts/scattering/targets/2QSP
...
def main(par_idx,ground_truth_pdb):
    ...
    if ELECTRONIC_ONLY:
        XFEL_OBJ=XFEL
    experiment1 = XFEL_OBJ(exp_name1,energy,**exp_qwargs)
    experiment2 = XFEL_OBJ(exp_name2,energy,**exp_qwargs)
    # Create Crystals
    ...
    if ELECTRONIC_ONLY:
        crystal_exp1 = Crystal(ground_truth_pdb,allowed_atoms,
                                is_damaged=first_crystal_is_damaged,CNO_to_N = CNO_to_N,S_to_N=S_to_N,
                                **crystal_qwargs)
        params=get_sim_params(plasma_sim_handle)[0]
        start_time=params["start_t"] 
        end_time=params["end_t"] 
        exp_qwargs["t_fineness"]=len(first_crystal.times)-1
    else:
        exp_qwargs["t_fineness"]=0   # 1 time point
    ...
    if laser_firing_qwargs["SPI"]:
        ...
        if not ELECTRONIC_ONLY:
            ...
            experiment1.fire_laser(start_time,end_time,plasma_sim_handle,sim_data_dir,crystal_exp1, results_parent_dir=results1_parent_folder, **laser_firing_qwargs)
        ...
        if exp_name2 != None:
            ...
            if not ELECTRONIC_ONLY:
                ...
                experiment2.fire_laser(start_time,end_time,plasma_sim_handle,sim_data_dir,crystal_undmged, results_parent_dir=results2_parent_folder, **laser_firing_qwargs)
