...
def make_some_plots(mol_name,sim_output_parent_dir, label,figure_output_dir, tot_charge=False,bound_ionisation=False,free=False,free_slices=False,bound_ionisation_bar=False,orbital_densities_bar=False,photo_rates = False,split_free=False,combined_split_free=False):
    ...
    load_specific_atoms = ["C","N","O","H"]# ["C","Fe_singleShell"]#["C","N","O","S","Gd_fast","Cl","Na"]#["Gd_fast"] #["C"]#["Fe_singleShell","C"] #None #["C","N","O"]
    ...
    if tot_charge: 
        ...
        if letter == "a":
            ...
            ymax = 7
        ax = pl.plot_tot_charge(colours=abdullah_colors,ylim=[0,ymax],every=1,charge_difference=False,legend_loc="best",legend_frame=False,profile_height_factor=0.63,legend_kwargs=legend_kwargs)  
        
        for i,a in enumerate(load_specific_atoms):
            abd_data = np.genfromtxt(f"scripts/abdullah_data/{letter}_{a}.csv",delimiter=',',skip_header=1)
            X = abd_data[:,0] - 14
            Y = abd_data[:,1]

            ax.scatter(X,Y,label=a+" $\\texttt{XMDYN}$",color=abdullah_colors[i],s=1)
    ...
    if TIGHT_LAYOUT:
        ...
        pl.fig.subplots_adjust(left=0.215, bottom=0.22, right=0.995, top=0.945, wspace=0.2, hspace=0.4) # Custom 
        if VERTICAL_MODE:
