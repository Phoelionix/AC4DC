...
void ElectronRateSolver::sys_ee(const state_type& s, state_type& sdot) {
    ...
    //////applyDeltaF() collapsed loop//////
        //
        //double F_copy [Distribution::size] = {0};
    

        #ifdef TRACK_SINGLE_CONTINUUM
        const size_t flattened_count = Distribution::size;
        #else
        const size_t flattened_count = Distribution::size*(Distribution::num_continuums-1); // all continuums except the total.
        #endif
        double F_copy_flattened[flattened_count] ;
        
        //#pragma omp parallel for schedule(dynamic) nowait
        Eigen::VectorXd  u_flattened(flattened_count);
        size_t iter = 0; 
        #pragma omp parallel for num_threads(threads) 
        #ifdef TRACK_SINGLE_CONTINUUM
        for (size_t _c = 0;_c < Distribution::num_continuums; _c++){
        #else
        // Iterate through the continuum corresponding to each element's initiated cascades and add separately. 
        for (size_t _c = 1;_c < Distribution::num_continuums; _c++){
        #endif 
            for (size_t i=0; i<Distribution::size; i++) {
                u_flattened[iter] = vector_of_u[_c][i];
                iter++;
        }

        #pragma omp parallel for num_threads(threads) reduction(+ : F_copy_flattened)
        for (size_t i=0; i<flattened_count; i++) {
                F_copy_flattened[i]+= u_flattened[i];
            }
        }
        for (;_c < Distribution::num_continuums; _c++) {
            for (size_t i=0; i<Distribution::size; i++) {
                s.F[0][i]+=F_copy_flattened[iter];
                #ifdef TRACK_SINGLE_CONTINUUM
                s.F[_c][i]+=F_copy_flattened[iter];
                #endif
                iter++;
            }
        }
