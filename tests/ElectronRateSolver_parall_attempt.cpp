...
void ElectronRateSolver::execute_solver(ofstream & _log, const std::string& tmp_data_folder) {
    ...
    decay_processes_time, bound_EII_time, bound_TBR_timeA,bound_TBR_timeB, transport_time, eii_time_free, tbr_time_free,
    ...
    "decay processes", "Loss due to EII","Loss due to TBR off diag","Loss due to TBR diag", "bound-e transport", "get_Q_eii()", "get_Q_tbr()",
    "Q_ee()", "applyDeltaF()"
    };
...
void ElectronRateSolver::sys_bound(const state_type& s, state_type& sdot, state_type& s_bg ,const double t) {
    ...
    for (size_t a = 0; a < s.atomP.size(); a++) {
        ...
        for (;_c < Distribution::num_continuums; _c++){
            ...
            for (size_t n=0; n<N; n++) {                
                for (size_t init=0;  init<RATE_EII[a][n].size(); init++) {
                    for (auto& finPair : RATE_EII[a][n][init]) {
                        double tmp = finPair.val*s.F[_c][n]*P[init];
            ...
            k_vector.resize(num_k);

            size_t num_n_m_init = 0;
            size_t tmp_idx = 0;
            for (size_t n=0; n<N; n++) {
                for (size_t m=n+1; m<N; m++) {
                    k_vector[tmp_idx] = N + (N*(N-1)*0.5) - (N-n)*(N-n-1)*0.5 + m - n - 1;
                    num_n_m_init += RATE_TBR[a][k_vector[tmp_idx]].size();
                    tmp_idx++;
                }
            }
            std::vector<std::vector<size_t>> n_m_init_vectors;
            n_m_init_vectors.resize(num_n_m_init);
            std::vector<size_t> large_k_vector;
            large_k_vector.resize(num_n_m_init);
            tmp_idx = 0;
            size_t k_idx = 0;
            for (size_t n=0; n<N; n++) {
                for (size_t m=n+1; m<N; m++) {
                    const size_t k = k_vector[k_idx];
                    k_idx++;
                    for (size_t init=0;  init<RATE_TBR[a][k].size(); init++) {
                        n_m_init_vectors[tmp_idx] = std::vector<size_t> {n,m,init};
                        large_k_vector[tmp_idx] = k;
                        tmp_idx++;
                    }

                }
            }
            assert(tmp_idx==num_n_m_init);

            size_t num_all = 0;
            size_t large_k_idx = 0;
            for (auto& n_m_init: n_m_init_vectors){
                num_all+=RATE_TBR[a][large_k_vector[large_k_idx]][n_m_init[2]].size();
                large_k_idx++;
            }
            std::vector<std::pair<std::vector<size_t>*,SparsePair*>> n_m_init_vector_finpair;
            n_m_init_vector_finpair.resize(num_all);
            tmp_idx = 0;
            large_k_idx = 0;
            for (auto& n_m_init: n_m_init_vectors){
                for (auto& finPair : RATE_TBR[a][large_k_vector[large_k_idx]][n_m_init[2]]) {
                    n_m_init_vector_finpair[tmp_idx] = {&n_m_init, &finPair};
                    tmp_idx++;
                }
                large_k_idx++;
            }
            assert(tmp_idx==num_all);

            #pragma omp parallel for num_threads(threads) reduction(+ : Pdot_subst,sdot_bound_charge_tbr_subst)
            for (auto& n_m_init__finpair: n_m_init_vector_finpair){
                const auto& n_m_init = n_m_init__finpair.first;
                const auto& finPair = n_m_init__finpair.second;
            // for (size_t n=0; n<N; n++) {
            ...
            //     for (size_t m=n+1; m<N; m++) {
                    ...
                    //     for (auto& finPair : RATE_TBR[a][k][init]) {
                        //     #ifdef TRACK_SINGLE_CONTINUUM
                        //     double tmp = finPair.val*s.F[_c][n]*s.F[_c][m]*P[init]*2;   // _c = 0 (total continuum)
                        //     #else
                        //     double tmp = finPair.val*(s.F[_c][n]*s.F[0][m] + s.F[_c][m]*s.F[0][n])*P[init]; // as we are distinguishing the electron continuum of interest from the full continuum now.
                        //     #endif
                        //     Pdot_subst[finPair.idx] += tmp;
                        //     Pdot_subst[init] -= tmp;
                        ...
                        // }     
                #ifdef TRACK_SINGLE_CONTINUUM
                double tmp = (*finPair).val*s.F[_c][(*n_m_init)[0]]*s.F[_c][(*n_m_init)[1]]*P[(*n_m_init)[2]]*2;   // _c = 0 (total continuum)
                #else
                double tmp = (*finPair).val*(s.F[_c][(*n_m_init)[0]]*s.F[0][(*n_m_init)[1]] + s.F[_c][(*n_m_init)[1]]*s.F[0][(*n_m_init)[0]])*P[(*n_m_init)[2]]; // as we are distinguishing the electron continuum of interest from the full continuum now.
                #endif
                Pdot_subst[(*finPair).idx] += tmp;
                Pdot_subst[(*n_m_init)[2]] -= tmp;
                sdot_bound_charge_tbr_subst -= tmp;
            }
            ...
            for (size_t n=0; n<N; n++) {
                for (size_t init=0;  init<RATE_TBR[a][n].size(); init++) {
                    for (auto& finPair : RATE_TBR[a][n][init]) {
                        double tmp = finPair.val*s.F[_c][n]*s.F[0][n]*P[init];
                        Pdot_subst[finPair.idx] += tmp;
                        Pdot_subst[init] -= tmp;
                        sdot_bound_charge_tbr_subst -= tmp;
                    }
