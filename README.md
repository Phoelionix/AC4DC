...
6. Electrons generated by atomic processes are instantaneously spread in energy space to a width of order 10eV.
...
5. Refactor to decouple atomic physics from rate equation solving
  - As of recent work it's feasible to fully simulate targets with period 3 (heavy) atoms without making the orbital-as-shell approximation - the only barrier is that bound rate calculations become the dominant computational drain with adaptive grid updates. Separately calculating the cross-section coefficients from the atomic physics and storing them in a table for a finely-spaced mesh of energies (probably logarithmic), then snapping/restricting knot energies to that mesh, is a very achievable way to solve this.
